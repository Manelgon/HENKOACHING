...
import { convertirLeadACliente } from '@/actions/clientes'
import type { EstadoCliente, ServicioContratado, TarifaTipo } from '@/lib/supabase/database.types'
import { ESTADOS_CLIENTE, SERVICIOS, TARIFAS } from '@/features/clientes/components/estados'
...
export default function ConvertirClienteModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await runAction(
      'Convirtiendo lead en cliente',
      () => convertirLeadACliente(lead.id, {
        nombre: form.nombre,
        email: form.email,
      ...
      { successMessage: '¡Cliente creado! El lead se ha movido a Clientes' },
    )
    if (result.ok) onConverted()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      ...
      >
        <div className="px-6 md:px-8 py-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            <p className="font-roxborough text-2xl text-gray-900">Convertir a cliente</p>
            <p className="font-raleway text-sm text-gray-500">{lead.nombre} — {lead.email}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 md:px-8 py-6 space-y-6">
          ...
          {/* Datos fiscales */}
          <Section title="Datos fiscales (para facturación)">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Input label="Web de la empresa" value={form.web_url} onChange={(v) => setForm({ ...form, web_url: v })} placeholder="https://…" />
          ...
          <div className="border-t border-gray-100 pt-5 flex flex-col-reverse md:flex-row gap-3 sticky bottom-0 bg-white -mx-6 md:-mx-8 px-6 md:px-8 -mb-6 pb-6">
            <button
              type="button"
              onClick={onClose}
